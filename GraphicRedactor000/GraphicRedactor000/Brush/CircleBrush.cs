    ...
    public class CircleBrush : IBrush
    {
            ...
            for (int i = x1; i <= x2; i++)
            {
                for (int j = y1; j <= y2; j++)
                {
                    if ((i - xCenter) * (i - xCenter) + (j - yCenter) * (j - yCenter) <= radius * radius)
                        Canva.GetCanva.SetPixel(i, j, BrushColor);
                }
            }
        }
        ...
        private void DrawBorder(int x, int y)
        {
            ...
            for (int i = x1 + margin; i <= x2 - margin; i++)
            {
                ...
                Canva.GetCanva.SetPixel(i, (int)(yCenter + sqrt), BrushColor);
                Canva.GetCanva.SetPixel(i, (int)(yCenter + sqrt - 1), BrushColor);
                Canva.GetCanva.SetPixel(i, (int)(yCenter - sqrt), BrushColor);
                Canva.GetCanva.SetPixel(i, (int)(yCenter - sqrt + 1), BrushColor);
            }
            //проходим по оси y
            for (int j = y1 + margin; j <= y2 - margin; j++)
            {
                ...
                Canva.GetCanva.SetPixel((int)(xCenter + sqrt), j, BrushColor);
                Canva.GetCanva.SetPixel((int)(xCenter + sqrt - 1), j, BrushColor);
                Canva.GetCanva.SetPixel((int)(xCenter - sqrt), j, BrushColor);
                Canva.GetCanva.SetPixel((int)(xCenter - sqrt + 1), j, BrushColor);
            }
        }
